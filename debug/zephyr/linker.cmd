...
MEMORY
    ...
    SRAML ( rw ) : ORIGIN = (536805376), LENGTH = (65536)
...
/DISCARD/ :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    SRAML (NOLOAD) : { __SRAML_start = .; KEEP(*(SRAML)) KEEP(*(SRAML.*)) __SRAML_end = .; } > SRAML __SRAML_size = __SRAML_end - __SRAML_start; __SRAML_load_start = LOADADDR(SRAML);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
